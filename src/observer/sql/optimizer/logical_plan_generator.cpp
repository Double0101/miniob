    ...
    UpdateStmt *update_stmt, std::unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  UpdateLogicalOperator *update_operator = new UpdateLogicalOperator(table, update_stmt->update_values());

  std::vector<Field> fields;
  for (UpdateValue uv : update_stmt->update_values()) {
    fields.push_back(uv.field());
  }
